...
def get_max_length(train, val, test):
	...
	for file in os.listdir(test):
		if file.endswith(".npy"):
			mididat = np.load(os.path.join(test, file))
			maxmium = max(maximum, np.shape(mididat)[0])
			
	
def dir_to_files(folder):
	'''
	Retrieves all .npy files from directory folder
	:return: Python list of arrays
	'''
	arr = []
	i = 0
